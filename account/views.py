...
def lifting(request):
    ...
    latest_price_subquery = Doprice.objects.filter(product_id=OuterRef('id')).order_by('-added_on').values('price')[:1]
        ...
        if 'save_price' in request.POST:
            ...
            for product in products:
                ...
                if qty and total:
                    ...
                    product.current_price = Decimal(request.POST.get(f'{product.id}_price'))
                    product.save()

                    existing_lifting = Lifting.objects.filter(Q(product=product) & Q(date=request.POST.get('date'))).first()
                    if existing_lifting:
    ...
    return render(request, 'account/lifting.html', {'suppliers': suppliers, 'brands': brands, 'products': products})
...
def display_lifting_cart(request):
    ...
    products = []
    total_sum = Decimal('0')
    date = request.session.get('lifting_date', None)  # Add this line
    invoicing = request.session.get('lifting_invoicing', None)  # Add this line
    ...
    return render(request, 'account/liftingcart.html', {'products': products, 'total_sum': total_sum})
...
def finalize_lifting_cart(request):
    ...
    return redirect('lifting')  # Redirect to the desired page after finalizing
...
def sales(request):
    ...
    latest_price_subquery = Sellprice.objects.filter(product_id=OuterRef('id')).order_by('-added_on').values('price')[:1]
    ...
    next_mem_number = Sales.objects.all().aggregate(Max('mem_number'))['mem_number__max']
    if next_mem_number:
        next_mem_number += 1
    else:
        next_mem_number = 1
    ...
    if request.method == 'POST':
        ...
        if 'save_price' in request.POST or 'update_price' in request.POST:
            ...
            cart = {}  # Clear the cart before adding new products
            ...
            for product_id in selected_product_ids:
                ...
                product = get_object_or_404(Product, id=product_id)  # Updated line
                ...
                product_data = {
                    'id': product_id,
                    'qty': qty,
                    'price': products.get(id=product_id).current_pricee,  # Modified line
                }
    ...
    return render(request, 'account/sales.html', {'suppliers': suppliers, 'brands': brands, 'products': products, 'next_mem_number': next_mem_number})
...
def salescart(request):
    ...
    # Get sales_date and sales_mem_number from the session
    ...
    for product_id, product_data in cart.items():
        ...
        product_data['total_amount'] = float(product_data['qty']) * float(product_data['price'])
        product_data['total_commission'] = float(product_data['qty']) * float(product.commission)
        product_data['current_pricee'] = product_data['price']  # Set the sales price from the cart data
        products.append(product_data)
    ...
    return render(request, 'account/salescart.html', {'products': products, 'dsrs': dsrs, 'markets': markets, 'sales_date': sales_date, 'sales_mem_number': sales_mem_number})
...
def final(request):
    if request.method == 'POST':
        ...
        # Get date and memo number from session
        ...
        product_ids = [int(p.split('_')[1]) for p in request.POST if p.startswith('product_') and p.endswith('_qty')]
        ...
        with transaction.atomic():
            for product_id in product_ids:
                ...
                total_commission = float(request.POST[f'product_{product_id}_total_commission'])

                sales = Sales(
                    mem_number=memo_number,  # Add the memo_number field
                    product=product,
                ...
                # Update the product's quantity after the sale
...
def damage(request):
    ...
    # Get the next memo number
    next_mem_number = Damage.objects.all().aggregate(Max('mem_number'))['mem_number__max']
    if next_mem_number:
        next_mem_number += 1
    else:
        next_mem_number = 1

    # Annotate products with the total_qty
    products = products.annotate(total_qty=Sum('qty'))

    items = Damage.objects.select_related('product', 'Purchase_Price', 'sales_Price', 'dsr', 'market').all()  # Move this line outside the 'if' block
    ...
    return render(request, 'account/damage.html', {'suppliers': suppliers, 'brands': brands, 'products': products, 'next_mem_number': next_mem_number, 'items': items})
...
def add_to_damage(request):
    if request.method == 'POST':
        ...
        # Create a new memo number for this set of products
        ...
        for product_id in selected_product_ids:
            ...
            latest_sellprice_instance = product.sellprice_set.order_by('-added_on').first()
            damage = Damage.objects.create(
                ...
                qty=float(request.POST.get(f'price_{product_id}')),  # Convert the value to float
            )
            # Calculate and save the sales amount and purchase amount
            damage.save()

        # Filter Damage objects based on the selected products
        items = Damage.objects.select_related('product', 'Purchase_Price', 'sales_Price').filter(product_id__in=selected_product_ids, mem_number=new_mem_number)
        ...
        return render(request, 'account/damagecart.html', {'items': items, 'total_sales_amount': total_sales_amount, 'total_purchase_amount': total_purchase_amount})
...
def damagecart(request, mem_number):
    items = Damage.objects.select_related('product', 'Purchase_Price', 'sales_Price', 'dsr', 'market').filter(mem_number=mem_number)
    total_sales_amount = sum(item.sales_amount for item in items)
    total_purchase_amount = sum(item.purchase_amount for item in items)
    return render(request, 'account/damagecart.html', {'items': items, 'total_sales_amount': total_sales_amount, 'total_purchase_amount': total_purchase_amount})
@login_required
def bank_manage(request):
    ...
    # Calculate current_balance for each bank
    for bank in banks:
        total_amount = BankTransaction.objects.filter(bank=bank).aggregate(Sum('amount'))['amount__sum'] or 0
        bank.current_balance = total_amount

    current_balance = None
    bank_id = request.GET.get('bank')
    if bank_id:
        total_amount = BankTransaction.objects.filter(bank__id=bank_id).aggregate(Sum('amount'))['amount__sum'] or 0
        current_balance = total_amount

    return render(request, 'account/bank_manage.html', {'banks': banks, 'current_balance': current_balance})
@login_required
def supplierspayment(request):
    ...
    if request.method == 'POST':
        ...
        # Calculate the bank's current balance
        total_amount = BankTransaction.objects.filter(bank=bank).aggregate(Sum('amount'))['amount__sum'] or 0

        if total_amount >= amount:
            # Create a new SupplierPayment entry
    ...
    return render(request, 'account/supplierspayment.html', {'suppliers': suppliers, 'banks': banks, 'error_message': error_message})
@login_required
def acdiccount(request):
    ...
    if sr:
        return render(request, 'account/acdiccount.html', {'suppliers': suppliers, 'brands': brands, 'sr': sr, 'discount_setups': discount_setups})
    else:
        return render(request, 'account/acdiccount.html', {'suppliers': suppliers, 'brands': brands, 'discount_setups': discount_setups})
...
def chack(request):
    if request.method == 'POST':
        ...
        products = Product.objects.filter(supplier_id=supplier_id, brand_id=brand_id).order_by('name')
        
        # Fetch the sales rate for each product
        for product in products:
            try:
    ...
    return render(request, 'account/chack.html', {'suppliers': suppliers, 'brands': brands, 'products': products})
...
def success_page_view(request):
    return render(request, 'account/success_page.html')  # Replace 'account/success_page.html' with the actual template path
    
    
@login_required
def nast(request):
    return render(request, 'account/nast.html')
