    ...
    if request.method == 'POST':
        ...
        if lifting_items.exists():
            ...
            for lifting in lifting_items:
                ...
                product_qty_price_total_list.append((product, qty, price, total_price, lifting))
            ...
            # handle case where lifting not found
            pass
...
def update_lifting(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # recalculate the stock value based on the purchase price
            products = Product.objects.all()
            latest_do_price_subquery = Doprice.objects.filter(product_id=OuterRef('id')).order_by('-added_on').values('price')[:1]
            ...
            return JsonResponse({'status': 'success', 'html': render('report/stockreport_partial.html', context=context)})
...
def lifting_delate(request):
    if request.method == 'POST':
        ...
        if invoice_number:
            ...
            messages.success(request, f"Deleted all items associated with invoice number {invoice_number}")
        else:
...
def sales_delate(request):
    if request.method == 'POST':
        ...
        if memo_number:
            ...
            messages.success(request, f"Deleted all items associated with memo number {memo_number}")
        else:
...
def damage_delate(request):
    if request.method == 'POST':
        mem_number = request.POST.get('mem_number')  # Change 'memo_number' to 'mem_number'
...
def supplierpaymentupadte(request):
    ...
    return render(request, 'control/supplierpaymentsetup.html', {'supplier_payments': supplier_payments})
...
def update_supplier_payment(request):
    if request.method == 'POST':
        ...
        # Update the SupplierPayment amount
...
def damagealldelate(request):
    if request.method == 'GET':
        ...
        if from_date and to_date:
            # Define a Q object to filter by supplier, brand, and date range
            q_filter = Q()
            if supplier_id:
            ...
            # Delete the matching damage records
        ...
        return render(request, 'control/damagealldelate.html', {'suppliers': suppliers, 'brands': brands})

    # Handle other request methods (e.g., POST) or conditions if needed
...
@login_required
def brandandsupply_delete(request):
    suppliers = Supplier.objects.all()
    brands = Brand.objects.all()
    ...
    if request.method == "POST":
        ...
        if supplier_id:
            try:
                ...
                # Delete all products related to this supplier
                Product.objects.filter(supplier=supplier).delete()
                # Delete all brands related to this supplier
                Brand.objects.filter(supplier=supplier).delete()
                # Delete the supplier itself
                supplier.delete()
                messages.success(request, "সাপ্লাইয়ার এবং সংশ্লিষ্ট সব প্রোডাক্ট ও ব্র্যান্ড ডিলিট হয়েছে।")
            except Supplier.DoesNotExist:
                messages.error(request, "সাপ্লাইয়ার পাওয়া যায়নি।")
        ...
        if brand_id:
            try:
                ...
                # Delete all products related to this brand
                Product.objects.filter(brand=brand).delete()
                # Delete the brand itself
                brand.delete()
                messages.success(request, "ব্র্যান্ড এবং সংশ্লিষ্ট সব প্রোডাক্ট ডিলিট হয়েছে।")
            except Brand.DoesNotExist:
                messages.error(request, "ব্র্যান্ড পাওয়া যায়নি।")

        return redirect('brandandsupply_delete')

    return render(request, 'control/brandandsupply.html', {
        'suppliers': suppliers,
        'brands': brands,
    })
