...
from django.db.models import F, ExpressionWrapper, FloatField, Subquery, OuterRef, Case, When
from django.db.models.functions import Cast
from django.shortcuts import render
from django.db.models import Sum, OuterRef, Subquery, Q
from datetime import datetime
from django.db.models import F, ExpressionWrapper, FloatField, Subquery, OuterRef, Case, When
from django.db.models import Sum, Avg, F, FloatField, Case, When, Subquery, OuterRef,  ExpressionWrapper
...
def stockreport(request):
    ...
    latest_do_price_subquery = Doprice.objects.filter(product_id=OuterRef('id')).order_by('-id').values('price')[:1]
    products = products.annotate(do_price=Subquery(latest_do_price_subquery))

    latest_sell_price_subquery = Sellprice.objects.filter(product_id=OuterRef('id')).order_by('-id').values('price')[:1]
    ...
    if request.method == 'POST':
        ...
        another_date = request.POST.get('from-date2')  # New: Get Another Date
        ...
        if date:
            ...
            for product in products:
                sales_in_last_10_days = Sales.objects.filter(product=product, date__range=[ten_days_before, date_obj]).exists()
                product.not_sold_in_last_10_days = not sales_in_last_10_days  # Set a flag indicating whether the product was not sold in the last 10 days


            for product in products:
                today_sales = Sales.objects.filter(product=product, date=date_obj).aggregate(today_sales=Sum('quantity'))['today_sales']
                product.today_sales = today_sales if today_sales is not None else 0


            total_purchases = Lifting.objects.filter(Q(date__lte=date_obj) & Q(product__in=products)).values('product').annotate(total_purchase_qty=Sum('quantity'))
            total_sales = Sales.objects.filter(Q(date__lte=date_obj) & Q(product__in=products)).values('product').annotate(total_sales_qty=Sum('quantity'))
            ...
            for sale in total_sales:
                ...
                if product_id in product_stats:
                    product_stats[product_id]['total_sales_qty'] = sale['total_sales_qty']
                else:
                    product_stats[product_id] = {
            ...
            for product in products:
                if product.id in product_stats:
                    product.total_purchase_qty = product_stats[product.id]['total_purchase_qty']
                    product.total_sales_qty = product_stats[product.id]['total_sales_qty']
                    product.stock_on_date = product.total_purchase_qty - product.total_sales_qty
                else:
            ...
            for product in products:
                product.stock_value_do_price = product.stock_on_date * product.do_price if product.do_price else 0
                product.stock_value_sell_price = product.stock_on_date * product.sell_price if product.sell_price else 0

        if another_date:
            another_date_obj = datetime.strptime(another_date, '%Y-%m-%d').date()

            another_total_purchases = Lifting.objects.filter(Q(date__lte=another_date_obj) & Q(product__in=products)).values('product').annotate(total_purchase_qty=Sum('quantity'))
            another_total_sales = Sales.objects.filter(Q(date__lte=another_date_obj) & Q(product__in=products)).values('product').annotate(total_sales_qty=Sum('quantity'))
            ...
            for sale in another_total_sales:
                ...
                if product_id in another_product_stats:
                    another_product_stats[product_id]['total_sales_qty'] = sale['total_sales_qty']
                else:
                    another_product_stats[product_id] = {
                        'total_purchase_qty': 0,
                        'total_sales_qty': sale['total_sales_qty'],
            ...
            for product in products:
                if product.id in another_product_stats:
                    product.total_purchase_qty_another_date = another_product_stats[product.id]['total_purchase_qty']
                    product.total_sales_qty_another_date = another_product_stats[product.id]['total_sales_qty']
                    product.stock_on_another_date = product.total_purchase_qty_another_date - product.total_sales_qty_another_date
                else:
            ...
            for product in products:
                product.stock_value_do_price_another_date = product.stock_on_another_date * product.do_price if product.do_price else 0
                product.stock_value_sell_price_another_date = product.stock_on_another_date * product.sell_price if product.sell_price else 0
...
def liftingreport(request):
    ...
    if request.method == 'GET':
        ...
        # Order the liftings by invoicing
        liftings = liftings.order_by('invoicing')

    total_product_value = liftings.aggregate(total=Sum('total_amount'))['total'] or 0
    # ...
    if search_clicked:
        liftings = liftings.order_by('invoicing', 'product__name')  # Order by invoicing and product name
        grouped_liftings = {k: list(v) for k, v in groupby(liftings, key=lambda x: x.invoicing)}
        return render(request, 'report/liftingreportresult.html', {'suppliers': suppliers, 'brands': brands, 'grouped_liftings': grouped_liftings, 'total_product_value': total_product_value})
    else:
        return render(request, 'report/liftingreport.html', {'suppliers': suppliers, 'brands': brands})
...
def salesreport (request):
    suppliers = Supplier.objects.all()
    brands = Brand.objects.all()
    return render(request,'report/salesreport.html',  {'suppliers': suppliers, 'brands': brands})
...
def sales_report_result(request):
    if request.method == 'GET':
        ...
        sales_data = Sales.objects.filter(
            ...
            total_value=Sum('total_product_price')
        )

        # Calculate sales rate and add it to the sales_data queryset
        ...
        return render(request, 'report/salesreportresult.html', {'sales_data': sales_data, 'total_sum': total_sum})
    else:
...
def salesreport(request):
    ...
    brands = Brand.objects.all()
    dsr = Dsr.objects.all()
    sales_data = Sales.objects.all()  # Add this line
    return render(request, 'report/salesreport.html', {'suppliers': suppliers, 'brands': brands, 'dsrs': dsr, 'sales_data': sales_data})  # Add 'sales_data': sales_data
...
def dsrsalesreport(request):
    ...
    if request.method == 'GET':
        ...
        sales_data = sales_data.values(
            'product__name', 'product__size', 'sell_price__price'  # Add 'product__size'
        ).annotate(
            total_qty=Sum('quantity'),
    ...
    return render(request, 'report/dsrsalesreport.html', {
        'suppliers': suppliers,
        'brands': brands,
        'dsrs': dsrs,
...
def salesreport(request):
    ...
    brands = Brand.objects.all()
    dsr = Dsr.objects.all()
    market = Market.objects.all()  # Add this line
    sales_data = Sales.objects.all()
    return render(request, 'report/salesreport.html', {'suppliers': suppliers, 'brands': brands, 'dsrs': dsr, 'markets': market, 'sales_data': sales_data})
...
def market_salesreport(request):
    ...
    return render(request, 'report/market_salesreport.html', {
        'suppliers': suppliers,
        'brands': brands,
        'market': market,
        'sales_data': sales_data,
...
def damagereport(request):
    ...
    if request.method == 'POST':
        ...
        # Filter and group Damage records based on the selected supplier, brand, and date range
        damage_data = Damage.objects.filter(
            product__supplier_id=supplier_id,
            product__brand_id=brand_id,
            date__range=(from_date, to_date)
        ).values(
            'date', 'mem_number'  # Group by these fields
        ).annotate(
        ...
        # Save the filtered damage_data into session
        request.session['filtered_damage_data'] = serialized_damage_data

        # Redirect to damagereportwithinvoivce page
        return redirect('damagereportwithinvoivce')
        
    return render(request, 'report/damagereport.html', {'suppliers': suppliers, 'brands': brands})
...
def damagereportwithinvoivce(request):
    ...
    return render(request, 'report/damagereportwithinvoivce.html', {'damage_data': damage_data})


@login_required
def damagewithtable(request, mem_number):
    # Use mem_number to filter Damage objects and get the corresponding product details
    damage_data_for_table = Damage.objects.filter(mem_number=mem_number)
    return render(request, 'report/damagewithtable.html', {'damage_data_for_table': damage_data_for_table})
...
def damage_report_result (request):
    if request.method == 'GET':
        ...
        date__range=(from_date, to_date)
            ...
            total_qty=Sum('qty'),
            total_value=Sum('purchase_amount')  # Change this line
        )
        ...
        return render(request, 'report/damagereportresult.html', {'damage_data': damage_data, 'total_sum': total_sum})
    else:
...
def supplier_ledger_result(request):
    if request.method == 'POST':
        ...
        liftings = Lifting.objects.filter(Q(date__range=(from_date, to_date)) & Q(product__supplier_id=supplier_id)).order_by('date', 'invoicing')
        supplier_payments = SupplierPayment.objects.filter(Q(date__range=(from_date, to_date)) & Q(supplier_id=supplier_id))
        ...
        for lifting in liftings:
            lifting.invoice_total = sum(l.total_amount for l in liftings_grouped[(lifting.date, lifting.invoicing)])
        ...
        return render(request, 'report/supplierledgerresult.html', {'combined_data': combined_data, 'total_invoice_taka': total_invoice_taka, 'total_supplies_taka': total_supplies_taka, 'distance': distance})
    else:
...
def profit_loss_report(request):
    ...
    if request.method == 'GET':
        ...
        if supplier_id and brand_id and from_date and to_date:
            return HttpResponseRedirect(reverse('profitlossreportresult') + f'?supplier={supplier_id}&brand={brand_id}&from-date={from_date}&to-date={to_date}')
    
    return render(request, 'report/profitlossreport.html', {'suppliers': suppliers, 'brands': brands})
...
def profit_loss_result(request):
    ...
    latest_doprice_subquery = Doprice.objects.filter(product_id=OuterRef('product_id')).order_by('-added_on').values('price')[:1]
    ...
    sales = Sales.objects.select_related('product').filter(
        ...
        'product_id', 'product__name', 'product__size'
    ).annotate(
        total_quantity=Sum('quantity'),
        purchase_price=Subquery(latest_doprice_subquery),
        sales_price=Avg('sell_price__price'),
        distance=ExpressionWrapper(Avg('sell_price__price') - F('purchase_price'), output_field=FloatField()),
        total_commission=Sum('total_commission')  # Add this line to include total_commission in the queryset
    ).annotate(
        profit=ExpressionWrapper(Case(
            When(distance__gte=0, then=F('distance') * F('total_quantity')),
            default=Cast(0, output_field=FloatField()),
        ), output_field=FloatField()),
        loss=ExpressionWrapper(Case(
    ...
    total_commission = sum(sale['total_commission'] for sale in sales)  # Now it should be available in the queryset
    net_profit = total_profit - (total_commission + total_loss)

    return render(request, 'report/profitlossreportresult.html', {
        'sales': sales,
...
def discountreport(request):
    ...
    if request.method == 'POST':
        ...
        # Filter Acdiccount objects based on the selected criteria
        ...
        return render(request, 'report/discountreportresult.html', {'acdiccounts': filtered_acdiccounts, 'total_amount': total_amount})
    else:
        return render(request, 'report/discountreport.html', {'suppliers': suppliers, 'brands': brand, 'discount': discount})
...
def costreport(request):
    ...
    if request.method == 'POST':
        ...
        # Filter Dailycost objects based on the selected criteria
        ...
        total_other_cost = filtered_dailycost.aggregate(Sum('othercost'))['othercost__sum']

        return render(request, 'report/costreportresult.html', {'dsrs': dsrs, 'dailycost': filtered_dailycost, 'total_car_cost': total_car_cost, 'total_dsr_bill': total_dsr_bill, 'total_toll': total_toll, 'total_other_cost': total_other_cost})
    else:
...
def dsrsalesreportinvoice(request):
    ...
    return render(request, 'report/dsrsalesreportinvoice.html', {'sales_data': sales_data})
@login_required    
...
def statementreportresultall(request):
    if request.method == 'POST':
        ...
        # Create a dictionary to store the balances for each collection manager
        collection_balances = {}
        ...
        for collection_man in collection_men:
            ...
            # Calculate the balance for this collection manager
...
def searchkg(request):
    ...
    suppliers = Supplier.objects.all()
    brands = Brand.objects.all()
    return render(request, 'report/searchkg.html', {'suppliers': suppliers, 'brands': brands})
from django.db.models import Sum
@login_required
def findkg(request):
    ...
    # Get all lift records in the date range filtered by supplier and brand
    ...
    # Create a list to hold your product data
    product_data = []

    # Loop through each unique product and get total lift quantity
    for product in unique_products:
        product_obj = Product.objects.get(id=product['product'])
        
        total_lifting_qty = lift_records.filter(product=product['product']).aggregate(Sum('quantity'))['quantity__sum'] or 0

        grams = GramSetup.objects.filter(product=product_obj).first()
        
        if grams:
            set_grams = grams.grams  # This is the static grams value you set previously
            total_grams = grams.grams * total_lifting_qty
            total_kg = total_grams / 1000
        else:
            set_grams = 0  # If grams value is not set, it will be 0
            total_grams = 0
            total_kg = 0
        
        product_data.append({
            'serial_no': ...,  # Continue to determine how to generate serial numbers
            'product_name': product_obj.name,
            'product_size': product_obj.size,
            'total_lifting_qty': total_lifting_qty,
            'set_grams': set_grams,  # Add this line to include the static grams value
            'grams': total_grams,
            'kg': total_kg,
        })
...
def assetviewrs(request):
    if request.method == "GET":
        ...
        return render(request, 'report/assetviewrs.html', {'transactions': transactions})
...
def order(request):
    ...
    if request.method == 'POST':
        ...
        if supplier_id and brand_id :
            products = Product.objects.filter(supplier_id=supplier_id, brand_id=brand_id)

            # Debug: Check if there are any lifting records
            has_lifting = Lifting.objects.filter().exists()
            print(f'Lifting records exist: {has_lifting}')

            # Debug: Check if there are any sales records
            has_sales = Sales.objects.filter().exists()
            print(f'Sales records exist: {has_sales}')

            # If records exist, then proceed to annotate
            if has_lifting and has_sales:
                total_lifting_subquery = Lifting.objects.filter(
                    product=OuterRef('pk'),
                ...
                total_sales_subquery = Sales.objects.filter(
                    product=OuterRef('pk'),
                ...
                products = products.annotate(
                    stock_on_date=Subquery(
                        total_lifting_subquery,
    ...
    return render(request, 'report/order.html', context)
